...
get_biomart_genes <- function(species, version=109, regions) {
  ensembl <- useEnsembl(biomart = "genes", dataset = species, version = version)
  BM.info <- getBM(attributes=c("ensembl_gene_id", "ensembl_transcript_id", "transcript_is_canonical"),
                   filters="chromosome_name",
                   values= unique(regions$seqnames),
                   mart=ensembl)
  gene_features <- getBM(attributes = c("ensembl_gene_id", "external_gene_name", "ensembl_transcript_id", "chromosome_name", "start_position", "end_position", "strand", "transcription_start_site"),
  ...
  gene_features <- gene_features %>% as_granges() %>% data.frame()

  overlap <- find_overlaps_within(as_granges(regions), as_granges(gene_features)) %>% data.frame() %>% group_by(ensembl_gene_id) %>% summarise(n_regions = n()) %>% data.frame()
  gene_features$n_regions <- overlap[match(gene_features$ensembl_gene_id, overlap$ensembl_gene_id), "n_regions"]
  gene_features <- gene_features %>% mutate(n_regions = coalesce(n_regions, 0), seqnames = paste0("chr", seqnames))
