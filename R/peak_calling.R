...
new_fn_peaks <- function(dm_results, gap_size=150) {
  ...
  peaks <- df %>%
    dplyr::mutate(number = seq_len(nrow(.)),
                  a = unsplit(lapply(split(.[,"meth_status"], .data$seqnames),
                                     function(x) {sequence(rle(x)$lengths)}),
                              .data$seqnames),
                  b = ifelse(dplyr::lead(.data$a) == .data$a, 0, .data$a),
                  multiple = ifelse(.data$b == 0, FALSE, TRUE)) %>%
    dplyr::filter(.data$dm == 1) %>%
    dplyr::mutate(gap_regions = ifelse(dplyr::lead(.data$number) != .data$number + 1,
                                       dplyr::lead(.data$number) - .data$number -1, NA),
           gap_width = ifelse(is.na(.data$gap_regions), NA,
                              dplyr::lead(.data$start) - .data$end)) %>%
    dplyr::mutate(new_one = ifelse(dplyr::lead(.data$a) < .data$a, "break", NA),
...
simplify_peaks <- function(peaks) {
  df <- peaks
  df <- df %>% dplyr::group_by(.data$seq) %>%
    dplyr::mutate(start = ifelse(.data$seq != 0, min(.data$start), .data$start),
  ...
  df <- df %>%
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(peak_id = seq_len(nrow(.)),
                  multiple_peaks = NA) %>%
    .[,c("peak_id", "Position", "seqnames", "start", "end", "width",
...
new_fn_gaps <- function(peaks, gap_size=150) {
  df <- peaks
  df <- df %>%
    dplyr::mutate(gap_width = ifelse(.data$gap_width < 0, 1e6, .data$gap_width),
                  less_gap = ifelse(.data$gap_width <= gap_size, "Yes", "No"),
                  a = sequence(rle(.data$less_gap)$lengths),
                  b = ifelse(dplyr::lead(.data$a) == .data$a, 0, .data$a),
                  multiple = ifelse(.data$b == 0, FALSE, TRUE)) %>%
    dplyr::mutate(end = ifelse(.data$multiple == FALSE & .data$less_gap == "Yes",
                               dplyr::lead(.data$end), NA),
                  n_regions_dm = ifelse(.data$multiple == FALSE &
                                          .data$less_gap == "Yes",
    ...
    dplyr::mutate(min_p_combo = ifelse(.data$multiple == FALSE & .data$less_gap == "Yes",
...
new_fn_p_combine <- function(peaks, gaps) {
  ...
  peaks.df <- peaks.df[,c("peak_id", "seqnames", "start", "end", "width",
                          "strand", "logFC_match", "FDR",
                          "multiple_peaks", "region_pos", "n_regions_dm")] %>%
    dplyr::mutate(n_regions_not_dm = NA,
           peak_id = paste0("PS_", .data$peak_id))
  gaps.df <- gaps.df[,c("peak_id", "seqnames", "start", "end", "width",
  ...
  df <- df[,c("peak_id", "seqnames", "start", "end", "width", "strand",
