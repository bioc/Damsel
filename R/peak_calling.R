...
peaks_jan_fn <- function(peaks) {
  ...
  df %>% mutate(seqnames = case_when(number < seqnames_pos[1] ~ "chr2L", number < seqnames_pos[2] ~ "chr2R", number < seqnames_pos[3] ~ "chr3L", number < seqnames_pos[4] ~  "chr3R", number < seqnames_pos[5] ~ "chr4", TRUE ~ "chrX"))
}
