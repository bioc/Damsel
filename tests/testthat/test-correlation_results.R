...
test_that("corr_heat: Output is message", {
  expect_message(corr_heatmap(process_bams(system.file("extdata", package = "Damsel"), regions = regions_gatc_drosophila_dm6, cores = 2)), "default spearman's method is used")#, ignore.case = TRUE)
})
...
test_that("corr_heat: Output is no error/message", {
  ...
  expect_no_message(corr_heatmap(df = process_bams(system.file("extdata", package = "Damsel"), regions = regions_gatc_drosophila_dm6, cores = 2), method = "spearman"))
})
