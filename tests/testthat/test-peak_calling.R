...
test_that("add_de:Output is error", {
  ...
  expect_error(add_de(edgeR_results(edgeR_set_up(process_bams(system.file("extdata", package = "Damsel"))[,c(1:6,7,10,8,11,9,12)])), regions = "A"), "Regions must be a data.frame")#, ignore.case = TRUE)
})
...
test_that("peaks:Output is error", {
  ...
  expect_error(aggregate_peaks(edgeR_results(edgeR_set_up(process_bams(system.file("extdata", package = "Damsel"))[,c(1:6,7,10,8,11,9,12)])), regions = 7), "Regions must be a data.frame")#, ignore.case = TRUE)
})

test_that("peaks:Output is message", {
  expect_message(aggregate_peaks(edgeR_results(edgeR_set_up(process_bams(system.file("extdata", package = "Damsel"))[,c(1:6,7,10,8,11,9,12)]))), "Default of drosophila dm6 regions used")
})
...
test_that("peaks:Output is not error/message", {
  ...
  expect_message(aggregate_peaks(edgeR_results(edgeR_set_up(process_bams(system.file("extdata", package = "Damsel"))[,c(1:6,7,10,8,11,9,12)])), regions = regions_gatc_drosophila_dm6))
})
