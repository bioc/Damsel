...
test_that("geom_peak: output is ggplot", {
    expect_s3_class(plot_counts_all_bams(readRDS(test_path("fixtures", "test_counts_df.rds")), seqnames = "chr2L", start_region = 1, end_region = 10000, n_col = 1) +
        geom_peak.new(aggregate_peaks(edgeR_results(edgeR_set_up(readRDS(test_path("fixtures", "test_counts_df.rds"))), regions = readRDS(test_path("fixtures", "regions.rds"))))), c("patchwork", "gg", "ggplot"))
})
...
test_that("geom_peak: Output is message", {
    expect_message(plot_counts_all_bams(readRDS(test_path("fixtures", "test_counts_df.rds")), seqnames = "chr2L", start_region = 430000, end_region = 450000, n_col = 1) +
        geom_peak.new(readRDS(test_path("fixtures", "test_peaks.rds"))), "No data available for this region")
})
...
test_that("geom_peak: Output is no error", {
    expect_no_error(plot_counts_all_bams(readRDS(test_path("fixtures", "test_counts_df.rds")), seqnames = "chr2L", start_region = 1, end_region = 10000, n_col = 1) +
        geom_peak.new(aggregate_peaks(edgeR_results(edgeR_set_up(readRDS(test_path("fixtures", "test_counts_df.rds"))), regions = readRDS(test_path("fixtures", "regions.rds"))))))
    expect_no_error(plot_counts_all_bams(readRDS(test_path("fixtures", "test_counts_df.rds")), seqnames = "chr2L", start_region = 1, end_region = 10000, n_col = 1) +
