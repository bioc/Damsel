...
test_that("geom_peak: Output is message", {
    expect_message(plotCounts(readRDS(test_path("fixtures", "test_counts_df.rds")), seqnames = "chr2L", start_region = 430000, end_region = 450000, n_col = 1) +
        geom_peak(readRDS(test_path("fixtures", "test_peaks.rds"))), "No data available for this region")
})

test_that("geom_peak: Output is no error", {
    expect_no_error(plotCounts(readRDS(test_path("fixtures", "test_counts_df.rds")), seqnames = "chr2L", start_region = 1, end_region = 10000, n_col = 1) +
    ...
    expect_no_error(plotCounts(readRDS(test_path("fixtures", "test_counts_df.rds")), seqnames = "chr2L", start_region = 1, end_region = 10000, n_col = 1) +
        geom_peak(readRDS(test_path("fixtures", "test_peaks.rds"))))
    expect_no_error(plotCounts(readRDS(test_path("fixtures", "test_counts_df.rds")), seqnames = "chr2L", start_region = 1, end_region = 10000, n_col = 1) +
        geom_peak(readRDS(test_path("fixtures", "test_peaks.rds")), peak.label = TRUE))
})
