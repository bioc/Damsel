...
test_that("Output is data frame", {
  expect_s3_class(process_bams(path_to_bams = system.file("extdata", package = "Damsel"), regions = regions_gatc_drosophila_dm6, cores = parallel::detectCores()), "data.frame")
})

test_that("Output is error", {
  expect_error(process_bams(regions = regions_gatc_drosophila_dm6), "Path to bam files required", ignore.case = TRUE)
...
test_that("No error/warning/message", {
  ...
  expect_no_message(process_bams(path_to_bams = system.file("extdata", package = "Damsel"), regions = regions_gatc_drosophila_dm6, cores = parallel::detectCores()))
})
